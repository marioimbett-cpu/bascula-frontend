...
export default function NuevoTicketPage() {
  ...
  const {
    ...
    defaultValues: { tipoMovimiento: "Compra", pesoEntradaKg: 0, pesoSalidaKg: 0, pesoDescontadoKg: 0, valorUnKg: 0, valorServicio: 20000 },
  });
  ...
  const esServicioBascula = tipoMovimiento === "Servicio de Báscula";

  const onSubmit = (values: TicketValidationFormValues) => {
    const { valorServicio, ...datosTicket } = values;
    const producto = productos?.data.find((p) => p.id === values.productoId);
    crearTicket.mutate(
      {
        ...datosTicket,
        productoId: esServicioBascula ? undefined : values.productoId,
        valorUnKg: esServicioBascula ? undefined : values.valorUnKg,
        pesoBrutoKg: pesoEntrada,
        pesoNetoKg: pesoNeto,
        valorTotal: esServicioBascula ? Number((values.valorServicio ?? 0).toFixed(2)) : Number((pesoNeto * (values.valorUnKg ?? 0)).toFixed(2)),
        nombreMaterialBascula: esServicioBascula ? "Servicio de báscula" : producto?.nombre ?? "",
        metodoCaptura: metodo === "OCR" ? "OCR" : "Manual",
        tipoAdjunto: metodo === "OCR" ? "Imagen" : metodo === "PDF" ? "PDF" : undefined,
        imagenTicketUrl: archivoUrl,
        estado: "Capturado",
        estadoValidacion: esServicioBascula ? "Validado" : "Pendiente",
      },
      {
        onSuccess: () => {
          reset();
    ...
    <div className="space-y-4">
      ...
      {tipoMovimiento === "Servicio de Báscula" && (
        <p className="text-xs text-muted-foreground">
          Solo pesaje: no se registra producto ni valor por kg. Se pesa, se registran los datos del
          cliente y se cobra directo el servicio — no genera orden de compra/venta ni movimiento de inventario.
        </p>
      )}
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
              ...
              {tipoMovimiento !== "Servicio de Báscula" ? (
                ...
                <FormField
                  id="valorServicio"
                  label="Valor del servicio de báscula"
                  required
                  error={errors.valorServicio?.message}
                  helpText="Se cobra directo al tercero por el pesaje — no hay producto ni compra/venta de por medio"
                >
                  <Input type="number" step="0.01" {...register("valorServicio")} />
                </FormField>
              )}

              <Button type="submit" className="w-full" isLoading={isSubmitting || crearTicket.isPending}>
                <Save className="h-4 w-4" aria-hidden="true" />
                {tipoMovimiento === "Servicio de Báscula" ? "Registrar pesaje y pago" : "Registrar ticket"}
              </Button>
            </form>
          </CardContent>
        </Card>
